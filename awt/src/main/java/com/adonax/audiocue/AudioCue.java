...
public class AudioCue implements AudioMixerTrack
{
	...
	public static AudioInputStream getSupportedAudioInputStream(AudioInputStream ais) {
		if (ais == null) throw new IllegalArgumentException("ais == NULL!");
		if (ais.getFormat().matches(audioFormat)) return ais;
		return new AudioInputStream(AudioSystem.getAudioInputStream(audioFormat, ais), audioFormat, ais.getFrameLength());
	}
	...
	public static enum PanType
	{
		...
		public final Function<Float, Float> left;
		public final Function<Float, Float> right;
	...
	private static float[] loadAudioInputStream(AudioInputStream ais) throws
			IOException
	{
		if (ais == null) throw new IllegalArgumentException("ais == NULL!");
	...
	{
		return (long)((cueFrameLength * 1_000.0)
				/ audioFormat.getFrameRate());
	}
	 ...
	 * pool of available instances when playing completes.
	 *
	 * @param volume - a {@code float} within the range [0, 1]
	 * @param pan    - a {@code float} within the range [-1, 1]
	 ...
	 * @see Type#START_INSTANCE
	 */
	public int play(float volume, float pan, double speed, int loop)
	{
	 ...
	 * @see #setMicrosecondPosition(int, long)
	...
	public long getMillisecondPosition(int instanceID)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
		...
		return (long) (cursors[instanceID].cursor * 1_000.0 / audioFormat.getFrameRate());
	}

	public long getMicrosecondPosition(int instanceID)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
		...
		return (long) (cursors[instanceID].cursor * 1_000_000.0 / audioFormat.getFrameRate());
	}

	public double getFractionalPosition(int instanceID)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
		...
		return cursors[instanceID].cursor / cueFrameLength;
	}
	 ...
	 * @param instanceID   - an {@code int} used to identify an
	 *                       {@code AudioCue} instance
	 * @param microseconds - an {@code long} in microseconds that
	 ...
	 * between two frames.
	 * <p>
	 * The input millisecond position will be clamped to a
	 * frame that lies within or is located at the start or end
	 * of the media data. When the instance is next started, it
	 * will commence from this position.
	 * <p>
	 ...
	 * @param instanceID   - an {@code int} used to identify an
	 *                       {@code AudioCue} instance
	 * @param milliseconds - an {@code long} in milliseconds that
	 *                       corresponds to a position in the
	 *                       audio media
	 * @throws IllegalStateException if instance is not active
	 * 		   or if instance is playing
	 * @see #getFramePosition(int)
	 */
	public void setMillisecondPosition(int instanceID,
									   long milliseconds)
	{
		if (!cursors[instanceID].isActive ||
				cursors[instanceID].isPlaying)
		...
		float frames = (audioFormat.getFrameRate() * milliseconds)
				/ 1000f;
		cursors[instanceID].cursor =
				Math.max(0,	Math.min(cueFrameLength, frames));
	};
	...
	public void setFractionalPosition(int instanceID, double normal)
	{
		...
		cursors[instanceID].cursor = (cueFrameLength) *
	 ...
	 * @throws IllegalStateException if instance is not active
	 * @see #setVolume(int, float)
	 */
	public float getVolume(int instanceID)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
	 ...
	 * @throws IllegalStateException if instance is not active
	 * @see #setPan(int, float)
	 * @see PanType
	 * @see #setPanType(PanType)
	 */
	public float getPan(int instanceID)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
	 ...
	 * @param instanceID - an {@code int} used to identify the
	 *                     {@code AudioCue} instance
	 * @param pan        - a {@code float} ranging from -1 to 1
	 * @throws IllegalStateException if instance is not active
	 * @see #setPan(int, float)
	 * @see PanType
	 * @see #setPanType(PanType)
	 */
	public void setPan(int instanceID, float pan)
	{
		if (!cursors[instanceID].isActive)
		{
			throw new IllegalStateException(name + " instance: "
