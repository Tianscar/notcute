...
import static com.ansdoship.a3wt.android.A3AndroidUtils.*;
...
public class AndroidA3GraphicsKit implements A3GraphicsKit {
    ...
    public A3Image createImage(final int width, final int height, final int type) {
        return new AndroidA3Image(Bitmap.createBitmap(width, height, imageType2BitmapConfig(type)));
    }
    ...
    public A3Image readImage(final File input, final int type) {
        ...
        try {
            ...
            final Bitmap bitmap = BitmapIO.read(input, imageType2BitmapConfig(type));
    ...
    public A3Image readImage(final A3Assets assets, final String input, final int type) {
        ...
        try {
            ...
            if (framedImage != null) return framedImage;
            final Bitmap bitmap = BitmapIO.read(((AndroidA3Assets)assets).assets, removeStartSeparator(input), imageType2BitmapConfig(type));
            if (bitmap == null) return null;
            else return new AndroidA3Image(bitmap);
        } catch (final IOException e) {
            return null;
    ...
    public A3FramedImage readFramedImage(final File input, final int type) {
        checkArgNotNull(input, "input");
        try {
            return FramedBitmapIO.read(input, imageType2BitmapConfig(type));
        } catch (final IOException e) {
            return null;
        }
    }
    ...
    public A3FramedImage readFramedImage(final A3Assets assets, final String input, final int type) {
        ...
        checkArgNotNull(input, "input");
        try {
            return FramedBitmapIO.read(((AndroidA3Assets)assets).assets, removeStartSeparator(input), imageType2BitmapConfig(type));
        } catch (final IOException e) {
            return null;
        }
    }
    ...
    public A3Coordinate createCoordinate() {
        return new AndroidA3Coordinate(new Point());
    }

    @Override
    public A3Coordinate createCoordinate(final int x, final int y) {
        return new AndroidA3Coordinate(new Point(x, y));
    }

    @Override
    public A3Dimension createDimension() {
        return new AndroidA3Dimension();
    }
    ...
    public A3Area createArea() {
        return new AndroidA3Area(new Rect());
    }

    @Override
    public A3Area createArea(final int x, final int y, final int width, final int height) {
        return new AndroidA3Area(new Rect(x, y, x + width, y + height));
    }
    ...
    public A3Area createArea(final A3Coordinate pos, final A3Dimension size) {
        ...
        return createArea(pos.getX(), pos.getY(), size.getWidth(), size.getHeight());
    }
