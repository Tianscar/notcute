...
import static com.ansdoship.a3wt.android.A3AndroidUtils.getAlignedBitmap;
import static com.ansdoship.a3wt.android.A3AndroidUtils.getBitmap;
import static com.ansdoship.a3wt.android.A3AndroidUtils.gifDrawable2FramedImage;
...
public class GifFBIOSpi implements FBIOServiceProvider {
    ...
    private A3FramedImage mRead(InputStream stream, Bitmap.Config config) {
        ...
        if (drawable == null) return null;
            ...
            return gifDrawable2FramedImage(drawable, config);
        }
    }
    ...
    public A3FramedImage read(ParcelFileDescriptor descriptor, Bitmap.Config config) throws IOException {
        try {
            return gifDrawable2FramedImage(new GifDrawable(descriptor.getFileDescriptor()), config);
        }
        catch (IOException e) {
            return null;
        }
    ...
    public A3FramedImage read(byte[] data, int offset, int length, Bitmap.Config config) {
        try {
            ...
            return gifDrawable2FramedImage(new GifDrawable(bytes), config);
        }
        catch (IOException e) {
            return null;
        }
    ...
    public A3FramedImage read(AssetManager assets, String asset, Bitmap.Config config) throws IOException {
        try {
            return gifDrawable2FramedImage(new GifDrawable(assets, asset), config);
        }
        catch (IOException e) {
            return null;
        }
    ...
    public A3FramedImage read(Resources res, int id, Bitmap.Config config) throws IOException {
        try {
            return gifDrawable2FramedImage(new GifDrawable(res, id), config);
        }
        catch (IOException e) {
            return null;
        }
    ...
    public A3FramedImage read(ContentResolver resolver, Uri uri, Bitmap.Config config) throws IOException {
        try {
            return gifDrawable2FramedImage(new GifDrawable(resolver, uri), config);
        }
        catch (IOException e) {
            return null;
        }
    ...
    public boolean write(OutputStream output, A3FramedImage im, String formatName, int quality) throws IOException {
        ...
        for (final A3Image i : im) {
            ...
            encoder.setDelay((int) clamp(image.time, Integer.MIN_VALUE, Integer.MAX_VALUE));
            if (!encoder.addFrame(getAlignedBitmap(getBitmap(image.bitmap, Bitmap.Config.RGB_565),
