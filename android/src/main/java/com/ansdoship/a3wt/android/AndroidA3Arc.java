...
import static com.ansdoship.a3wt.android.A3AndroidUtils.normalizeDegrees;
import static com.ansdoship.a3wt.android.A3AndroidUtils.relativeCCW;
import static com.ansdoship.a3wt.android.A3AndroidUtils.intersectsLine;
...
public class AndroidA3Arc implements A3Arc {
    ...
    public boolean contains(final float x, final float y) {
        ...
        final boolean inside = (relativeCCW(x1, y1, x2, y2, 2*normx, 2*normy) *
                relativeCCW(x1, y1, x2, y2, 0, 0) >= 0);
        return inarc != inside;
    }
    ...
    public boolean contains(final float x, final float y, final float width, final float height) {
        ...
        if (intersectsLine(origrect, xc, yc, xe, ye)) {
            return false;
        }
        angle += Math.toRadians(-getSweepAngle());
        xe = xc + halfW * Math.cos(angle);
        ye = yc + halfH * Math.sin(angle);
        return !intersectsLine(origrect, xc, yc, xe, ye);
    }
