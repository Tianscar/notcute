...
public class A3ColorUtils {

    private A3ColorUtils(){}
    ...
    public static int darker(final int color, final float factor) {
        return argb(alpha(color), (int) Math.max(red(color) * factor, 0), (int) Math.max(green(color) * factor, 0), (int) Math.max(blue(color) * factor, 0));
    }
