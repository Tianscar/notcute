...
public interface A3Map<K, V> extends Map<K, V> {
     ...
     * @since 1.8
     */
    default void forEach(A3Consumer<A3PairTuple<? super K, ? super V>> action) {
        ...
        for (Map.Entry<K, V> entry : entrySet()) {
            ...
            action.accept(new A3PairTuple<>(k, v));
        }
    }
     ...
     * @since 1.8
     */
    default void replaceAll(A3Function<A3PairTuple<? super K, ? super V>, ? extends V> function) {
        ...
        for (Map.Entry<K, V> entry : entrySet()) {
            ...
            v = function.apply(new A3PairTuple<>(k, v));
    ...
    default V computeIfAbsent(K key,
                              A3Function<? super K, ? extends V> mappingFunction) {
    ...
    default V computeIfPresent(K key,
                               A3Function<A3PairTuple<? super K, ? super V>, ? extends V> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        V oldValue;
        if ((oldValue = get(key)) != null) {
            V newValue = remappingFunction.apply(new A3PairTuple<>(key, oldValue));
            if (newValue != null) {
                put(key, newValue);
                return newValue;
            } else {
    ...
    default V merge(K key, V value,
        ...
        V oldValue = get(key);
        V newValue = (oldValue == null) ? value :
                remappingFunction.apply(new A3PairTuple<>(oldValue, value));
        if (newValue == null) {
            remove(key);
        } else {
            put(key, newValue);
